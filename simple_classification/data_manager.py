...
class RawDataLoader(object):
    ...
    def load_dataset(self, mode, stat, x_keys):
        ...
        x, y, names = self.make_X_and_Y(data_list, stat, x_keys)
        return x, y, names
...
class EmotionDataset(Dataset):
    ...
    def __getitem__(self, index):
        return self.x[index], self.y[index] - 1, self.names[index]
...
def get_dataloader(data_path, file_name, feature_keys):
    ...
    x_train, y_train, names_train = DL.load_dataset('train', STAT_TYPE, feature_keys)
    #x_valid, y_valid, names_valid = DL.load_dataset('valid', STAT_TYPE, feature_keys)
    x_test, y_test, names_test = DL.load_dataset('test', STAT_TYPE, feature_keys)

    train_set = EmotionDataset(x_train, y_train, names_train)
    #valid_set = EmotionDataset(x_valid, y_valid, names_valid)
    test_set = EmotionDataset(x_test, y_test, names_test)
