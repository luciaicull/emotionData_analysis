...
def make_X_Y(total_dataset):
    x = []
    y = []
    feature_key_list = [key for key in list(total_dataset[0]['scaled_statistics'].keys())
    ...
    for feature_data in total_dataset:
        ...
        scaled_statistics = feature_data['scaled_statistics']

        data = []
        for key in feature_key_list:
            data.append(scaled_statistics[key])
        x.append(data)
